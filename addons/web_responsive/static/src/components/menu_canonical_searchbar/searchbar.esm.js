...
export class AppsMenuCanonicalSearchBar extends Component {
    ...
    _searchMenus() {
        ...
        state.rootItems = fuzzyLookup(query, this.rootMenuItems, (item) => item.name);
        state.subItems = fuzzyLookup(query, this.subMenuItems, (item) => item.name);
    }
    ...
    _onKeyDown(ev) {
        ...
        if (code === "Escape") {
            ...
            if (this.hasItemsToDisplay && element) {
                ev.preventDefault();
                element.querySelector(".highlight > .search-item__link").click();
            }
        } else if (code === "Home") {
            this.state.offset = 0;
        } else if (code === "End") {
